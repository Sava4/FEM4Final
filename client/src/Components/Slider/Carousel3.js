...
//styled component внутри мар дает ворнинг !!!
// })
  ...
  const [text, setText] = useState([])
  useEffect(() => {
    fetch("http://localhost:5000/slides").then(res => {
      return res.json();
    }).then(json => {
      setText(json);
    });
  ...
  const CarouselImage1 = styled.div`
  background:url(${{...[...text][0]}.imageUrl}) no-repeat;
  height: 425px;
  opacity:1;
  `
  const CarouselImage2 = styled.div`          
  background:url(${{...[...text][1]}.imageUrl}) no-repeat;
  height: 425px;
  opacity:1;
  `
  const CarouselImage3 = styled.div`          
  background:url(${{...[...text][2]}.imageUrl}) no-repeat;
  height: 425px;
  opacity:1;
  `
    ...
    <div
      ...
      <Slider {...settings}>
        <div>
          ...
          {/* <img src={process.env.PUBLIC_URL + `${{...[...text][0]}.imageUrl}`} alt="slide1" /> */}
          {/* <img src={`${{...[...text][0]}.imageUrl}`}/> */}
          {/* <img src={{...[...text][0]}.imageUrl}/> */}
          {/* '/img1.jpg' */}
          {/* {console.log({...[...text][0]}.imageUrl)} */}
